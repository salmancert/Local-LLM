...
|---|---|---|
| `FOUNDRY_MODEL` | `qwen3-8b` | Foundry Local catalog alias used for chat completions. This default has moved twice for the same reason -- tool-calling reliability is what this app lives or dies on. `phi-4-mini` (3.8B) would claim it had edited a file without ever emitting a tool call; `qwen2.5-7b` fixed the worst of that; `qwen3-8b` is a generation newer at essentially the same size, with stronger agentic/tool-use behavior. Stronger if you have the hardware: `qwen3-14b`, `phi-4`, `qwen2.5-14b`, or `gpt-oss-20b` (Microsoft's own pick for agentic work, but effectively needs a capable GPU). Faster: `qwen3-4b`, `qwen3-1.7b`. If the alias you set isn't in your catalog -- or can't call tools -- the app says so and falls back to the best one that can |
...
| `CHROMA_COLLECTION` | `chat_memory` | Base ChromaDB collection name. A collection built with a different-sized embedding model is detected automatically and a `_<dimension>` suffixed one is used instead, so you shouldn't normally need this -- see Embeddings below |
| `ALLOW_USER_FOLDERS` | `false` | Set `true` to let the file tools read/write your `Desktop`, `Documents` and `Downloads` folders in addition to `workspace/`. Read the sandbox note under Tool Calling before enabling |
| `EXTRA_TOOL_DIRS` | *(none)* | Specific extra folders the file tools may read/write, separated by `;` on Windows or `:` elsewhere. Narrower than `ALLOW_USER_FOLDERS` |
...
**Switching backends is handled automatically.** A ChromaDB collection is locked to the vector size first written to it, and the backends differ (Foundry's embedding models are typically 768 or 1024-dim; the bundled fallback is 384). Writing 384-dim vectors into a collection built at 768 simply fails — which, before this was handled, killed memory and document search on every single request. Now the collection is chosen to match the active embedder: a compatible existing collection is reused as-is, and a mismatch transparently moves to a `<name>_<dimension>` collection. **The old collection is never modified or deleted**, so restoring the previous embedding model brings that history straight back. `CHROMA_COLLECTION` still lets you name the base collection, but you shouldn't need to touch it.
...
**Model selection is catalog-aware.** Foundry Local's catalog marks which models support tool calling (`supports_tool_calling`, shown as the `tools` task in `foundry model list`), and the app reads it: if your configured model isn't in the catalog, or can't call tools, it walks a preference list and picks the best one that can, logging what it chose and why. Falling back to a model without tool support would silently disable tools, MCP, Workforce and PDF proofreading, so that's skipped past — though if *nothing* in your catalog supports tools, you still get a working chatbot plus a warning rather than a dead app.

**A note on reasoning models.** Qwen3 (the default) reasons in `<think>...</think>` natively, whether or not Deep think is on. `split_thinking()` in `app.py` strips those blocks the same way it handles this app's own `<thinking>`/`<answer>` format — without that, raw chain-of-thought would be shown as the reply, read aloud by TTS, and written into memory. A block truncated by the token limit is handled too, rather than leaking a dangling `<think>` tag into the chat.
...
| `local__proofread_pdf` | Proofread a PDF *on its own*: saves a copy with issues highlighted right on the page (orange = spelling, pink = grammar), using PyMuPDF's exact text positions. Spelling is an offline dictionary lookup (`pyspellchecker` -- no network call, ever); grammar (wrong-word errors like "there" for "their", subject-verb agreement, tense -- things no dictionary can catch since every word involved is spelled correctly) is checked by asking Foundry Local itself, batched into as few local-model calls as possible rather than one per sentence. Grammar quality follows directly from whatever `FOUNDRY_MODEL` you have configured, and can be turned off (`check_grammar: false`) for a faster spelling-only pass; only offered at all if `pyspellchecker` is installed. See below for how the grammar pass works and its honest limitations. |
...
**How `local__proofread_pdf`'s grammar pass works:** spelling has an offline dictionary, but grammar doesn't have an equivalent -- "their" is spelled correctly whether or not it's the right word for the sentence, so catching that needs actual language understanding, not a lookup. Rather than add a new dependency for this (a real grammar checker like LanguageTool needs a Java runtime and a large one-time download), it reuses Foundry Local: the tool sends the document's pages to the model with an instruction to quote any grammar issue's exact wording, then locates that exact text on the page (`page.search_for`) to highlight it -- a finding is silently dropped rather than mis-highlighted if the model paraphrases instead of quoting verbatim. All of a document's pages are batched into as few model calls as possible (grouped under a character budget, capped at `_MAX_GRAMMAR_BATCHES` batches so a very long document can't turn into dozens of sequential local-model calls) rather than checked one paragraph at a time -- the same batching principle that fixed slow document ingestion above. Verified end to end with a stand-in local server (real Foundry Local doesn't run in this project's dev sandbox): both a homophone error ("their" for "they're") and a subject-verb agreement error were correctly found and highlighted, including one that wrapped across a line break in the PDF, with no false positives on proper nouns in the same document. Real-world grammar quality depends entirely on your configured `FOUNDRY_MODEL` -- a small model will miss subtler issues and can occasionally flag something that wasn't actually wrong; there's no way around that other than using a stronger model.

**Everything file-related above is sandboxed to `workspace/`** (created automatically, gitignored). Every path a tool touches is resolved and checked to stay inside it -- no `..` traversal, no absolute paths -- so a tool call can read or overwrite files there and nowhere else on your machine.

**Reaching files outside the workspace (opt-in).** "Proofread the PDF on my desktop" is a reasonable thing to ask, and by default it can't be done -- the desktop is outside the sandbox. Two environment variables widen it: `ALLOW_USER_FOLDERS=true` adds `Desktop`, `Documents` and `Downloads`, and `EXTRA_TOOL_DIRS` (`;`-separated on Windows, `:` elsewhere) adds specific folders. Files there are then addressed by folder name, e.g. `Desktop/report.pdf`, or by absolute path. The model is told which folders it can reach, so it stops guessing -- before this, it would tell you a desktop file "does not exist in the local workspace" and offer to copy it there, which it had no way to do.

Be deliberate about this: anything you add gets the same **read and write** access as the workspace, and the sandbox is what stops an LLM-decided tool call from touching the rest of your machine. Everything within an added folder is fair game, but escapes from it (`Desktop/../../..`) are still refused, as are absolute paths pointing anywhere you didn't allow. Prefer `EXTRA_TOOL_DIRS` with one specific folder over granting all three user folders. Archive extraction gets the same treatment against "zip-slip"/"tar-slip": every member's path is checked before extracting, since a crafted archive with a member named e.g. `../../.bashrc` would otherwise happily write outside the destination (confirmed this actually works as a real attack against `zipfile`/`tarfile` if you don't check first -- both extract wherever a member name points, no questions asked). All of this matters because tool calls are LLM-decided, and the model's context can include content from outside the conversation (a fetched web page, an MCP tool's output, an uploaded document, a downloaded archive) -- treat that the same as any other prompt-injection surface. The sandbox is the actual safety boundary here, not a suggestion to the model.
